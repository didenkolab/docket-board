...
- [x] The result passes [[DKT-6 docket check — validate a vault]].
...
On the second acceptance criterion: [[DKT-6 docket check — validate a vault]] does not exist yet, so "passes" means I ran its
...
placeholder survives rendering, which is a test. The criterion originally pointed at [[DKT-5 docket new — create a task with a valid key]];
...
content, which will drift. That became [[DKT-11 Stop the embedded templates from drifting away from this vault]].
